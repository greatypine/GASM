...
public class ChartStatDaoImpl extends BaseDAOHibernate implements ChartStatDao {
	...
	public Map<String, Object> queryrecommenduserlist(QueryConditions queryConditions) {
		...
		String sqlDemo = "select customer_id,mobilephone,area_no,employee_a_no,item1,item2,item3 from ds_important_develop_customer where platformid like '"+platformid+"'";
		List<Map<String,Object>> list = ImpalaUtil.executeGuoan(sqlDemo);
		pageInfo.setTotalRecords(list.size());
		int lastLine = pageInfo.getRecordsPerPage()*pageInfo.getCurrentPage()>list.size()?list.size()-1:pageInfo.getRecordsPerPage()*pageInfo.getCurrentPage();
		List<Map<String,Object>> listDemo = list.subList(pageInfo.getRecordsPerPage()*(pageInfo.getCurrentPage()-1), lastLine);
