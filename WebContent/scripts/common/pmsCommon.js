...
/**
 * 初始化收起的三角号建议在页面加载期间调用该方法
 */
function initToggleNaviPict(){
 ...
 * add 郭大大 2013.7.25
 * @param num  数字或字符串
 * @param precision 要显示的精度 默认为2
 * @param fill 是否补0  默认为true
 * @returns {*} 处理后的字符串
 */
function upFormatNum(num,precision,fill) {
  ...
  if (isNaN(num.toString()) || isNaN(precision.toString())) {
    alert("输入的数值格式不正确，请修改。");
    return "";
  }
  ...
  //去除后面无用的0
  var reg=new RegExp("\\.[1-9]{0,}0{1,}$");
  if (strNum.search(reg)>-1) {
    var reg2=new RegExp("0{1,}$");
    strNum=strNum.replace(reg2,"");
  }
  //去除最后无用的.
  var reg3=new RegExp("\\.$");
  strNum=strNum.replace(reg3,"");

  //小数点位置
  var pIndex=strNum.indexOf(".");
  //当前精度
  var cprecistion=0;
  if (pIndex>-1) {
    cprecistion=strNum.length-(pIndex+1);
  }
  //如果当前精度小于等于指定精度，并且不需要填充，直接返回原数值
  if (cprecistion<=precision && !fill) return strNum;

  //如果当前精度小于等于指定精度，并且需要填充，则在后面填充指定的0,并返回
  if (cprecistion<=precision && fill) {
    if (cprecistion==0) strNum+=".";
    for (var i=0;i<precision-cprecistion;i++) {
      strNum+="0";
    }
    return strNum;
  }
  ...
  //如果当前精度大于指定精度
  if (cprecistion>precision) {
    var intVal=parseFloat(strNum) *(Math.pow(10,precision));
    intVal=Math.ceil(intVal);
    var valStr=intVal.toString();
    var valStr=valStr.substr(0,valStr.length-precision)+"."+valStr.substr(valStr.length-precision);
    if (valStr.indexOf(".")==0) valStr="0"+valStr;
    if (!fill) {
      //如果小数位有后置的无用0，则去除
      var reg=new RegExp("\\.[1-9]{0,}0{1,}$");
      if (valStr.search(reg)>-1) {
        var reg2=new RegExp("0{1,}$");
