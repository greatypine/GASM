...
* @author       : ruanqingfeng
* @version      : 上传组件2.0版本
* @desc         : （新增功能：1:一个页面允许多个上传组件table，多个组件被维护在fileUploadTablesDIY数组变量中，可通过数组索引到某个uploadTable）
* @desc         : （新增功能：2:支持表格改变属性后重新渲染）
* @for example  : 将本js引入后，在需要显示组件的地方加入<div class="uploadTable"></div>即可
*/

var FileUploadTableDIY = function(_cfg){
	this.requestString="";
	...
	this.renderTo = "uploadDIYTable";//表格渲染的容器Class
	this.rowInitCnt = 0;//初始化时需要默认构造几行
	this.showHeader = false;//初始化时是否显示表头
	this.maxRows    = 50;//允许添加最大行数
	this.tableWidth = "100%";//表格宽度
	this.showAddButton = false;//是否显示"增加"按钮
	this.tableColumnsWidth = ["4%","15%","20%","5%","30%","20%","6%"];//表格宽度设置
	this.tableColumnsShow  = [true,true,true,true,true,true,true];//表格的列显示配置（设置false，将不显示该列）
	this.tableColumnsType  = ["rownum","item","label","label","text","file","button"];//列类型
	this.tableColumns      = ["序号","文件种类","文件名称","类型","备注","上传附件","操作"];//表头（不可修改）
	...
	*               : fileUploadTablesDIY[0].readerTable(data)
	*/
	this.readerTable = function(_data){
		this.showAddButton = false;
		this.tableColumnsShow = [true,true,true,true,true,false,false];
	  	this.tableColumnsType = ["rownum","items","label","label","label","file","button"];
	...
	*/
	this.createRow = function(){
		...
		var tr = $("<tr name='uploadTr' class='uploadTableRowLineHeight'></tr>");
		...
		for(var i=0;i<this.tableColumns.length;i++){
			...
			if(this.tableColumnsType[i]=="rownum"){
				alignWord = "center";
			} else if(this.tableColumnsType[i] == "item") {
				alignWord = "left";
			...
			if(this.tableColumnsType[i]=="rownum"){
				...
				if (this.items != "undefined" && this.items != null) {
					...
					if (this.items[idx] != null) {
						...
						td.append(title);
						var req = "";
						if (this.items[idx].required) {
							req = "validate=\"required:true\"";
							td.append(":<span class=\"red\">*</span>");
						} else {
							td.append(":");
						}
						td.append("<input type=\"hidden\" value=\"\" name=\"item\" id=\"item"+idx+"\" /> ");
						td.append("<input type=\"hidden\" value=\"\" name=\"itemVal"+idx+"\" id=\"itemVal"+idx+"\" "+req+" /> ");
						td.addClass("uploadFileTableDataCenter");
					}
				}
			} else if(this.tableColumnsType[i]=="label"){
				...
				var fileBtn = $("<input isUploadTable=true type='"+this.tableColumnsType[i]+"' class='fileInputWidth inputtext_dis' name='attach"+this.tableIndex+(this.delFileBtnIndex)+"'id='attachDIY"+this.tableIndex+(this.delFileBtnIndex++)+"'/>");
				var _this = this;
				...
				var value = "";
				value="class='uploadFileTableBtn' value='清除'";
				td.addClass("uploadFileTableDataCenter");
				...
				delBtn.bind("click",function(){
					tr.children().eq(1).find("input").val("");
					tr.children().eq(2).html("");
					tr.children().eq(3).html("");
					tr.children().eq(4).find("input").val("");
	...
	*/
	this.upload = function(_file,_tr){
		...
		var item = _tr.children().eq(1);
		var fileNameTd = _tr.children().eq(2);
		var fileTypeTd = _tr.children().eq(3);
		var fileComTd = _tr.children().eq(4);
		var packageTd = _tr.children().eq(5);
		var attach = _file.id;
		...
		(
			{
				...
				success: function (data, status)
				{
					...
					var idx = $(_tr).index();
					item.find("input").val(_this.items[idx].title);
	...
	*/
	this.getValues = function(){
		...
		for(var i=1;i<tableTrs.length;i++){
			...
			var trValue = {
				attachmentId:aId,
				item:$(curTr.children().eq(1)).find("input").val(),
				fileName:$(curTr.children().eq(2).children().eq(0)).html(),
				fileType:$(curTr.children().eq(3)).html(),
				attachmentComment:$(curTr.children().eq(4)).find("input").val(),
	...
	this.setValues = function(_data){
		if (!this.showTable) {
			for(var i=0;i<_data.length;i++){
				...
				// 文件项目类型
				var t_c_1 = $(tr.children().eq(1));
				
				if (obj.item != null && obj.item != "null" && obj.item != "undefined") {
					t_c_1.html(obj.item + ":");					
				}
				
				$(tr.children().eq(2)).append(htmlObj);
				
				$(tr.children().eq(3)).html(eval("obj."+this.dataFieldName.fileType));
				var t_c_0 = $(tr.children().eq(4));
				if(t_c_0.find("input").length!=0){
					t_c_0.find("input").val(eval("obj."+this.dataFieldName.attachmentComment));
					t_c_0.find("input").removeClass("inputtext_dis");
					t_c_0.find("input").addClass("inputtextshort");
			...
			for (var j=0; j<rowArr.length; j++) {
				var tr = rowArr[j];
				var itemInput = this.items[j].title;
				for(var i=0;i<_data.length;i++){
					var obj = _data[i];
					if (obj.item != itemInput) {
						continue;
					}
					//var	tr = this.createRow(this.fileUploadTable);
					$(tr.children().eq(0)).find("input").val(eval("obj."+this.dataFieldName.attachmentId));
					...
					if(t_c_1.find("input").length!=0){
						t_c_1.find("input").val(eval("obj."+_this.dataFieldName.item));
					}
...
var fileUploadTablesDIY = [];
var uploadTableItemsDIY = {};
$(function(){
	$("div[class*='uploadDIYTable']").each(function(i,element){
		var defaultsDIY = $(this).attr("defaultsDIY");
		if(defaultsDIY==null||defaultsDIY==""){
			defaultsDIY = "{}";	
		}
		eval("var defaultsDIY ="+defaultsDIY);	
		defaultsDIY.tableIndex = i;
		defaultsDIY.renderToDiv= $(element);
		var objDIY = new FileUploadTableDIY(defaultsDIY);
		fileUploadTablesDIY.push(objDIY);
		uploadTableItemsDIY[$(this).attr("id")] = objDIY;
	});
})
