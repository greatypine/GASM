...
/** Join, listen and subscribe. */
	joinListen: function(aSubject, usercode) {
...
/** Out Publish to subject. */
	publishOut: function(aSubject, theQueryArgs) {

		var query = 'p_subject=' + aSubject;
		if (theQueryArgs) {
			query = query + '&' + theQueryArgs;
		}
		PL._doRequest('publishOut', query);
	},	
...
function p_join_listen(aSubject, usercode) {
	PL.joinListen(aSubject, usercode);
}
...
function p_publish_out(aSubject, nvPairs) {
	var args = p_publish_out.arguments;
