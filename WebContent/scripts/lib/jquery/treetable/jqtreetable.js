...
*/
(function(jq){
  jq.fn.jqTreeTable=function(map, options, queryid){
    var opts = jq.extend({openImg:"",shutImg:"",leafImg:"",lastOpenImg:"",lastShutImg:"",lastLeafImg:"",vertLineImg:"",blankImg:"",collapse:false,column:0,striped:false,highlight:false,state:true,isleaf:false},options),
    ...
    collapseKids = function(num, last){//Recursively collapses all children and their children and change icon
      ...
      for (var x=0, xl=mapa[num].length;x<xl;x++){
        var mnx = mapa[num][x];
		var queryidObj = $("#"+queryid);
		var bbb = $('tr', queryidObj);
		var aaa = jq("#"+tid+mnx).parents("tr");
		for(var i=0;i<bbb.length;i++){
			for(var y=0;y<aaa.length;y++){
				if(bbb[i] == aaa[y]){
					$(aaa[y]).addClass("collapsed");
				}
			}
		}
