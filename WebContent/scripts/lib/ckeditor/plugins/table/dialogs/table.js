...
(function() {
    function r(a) {
        ...
        l = 0,
        k = 0,
        m, e = a.$.rows.length; k < e; k++) {
            m = a.$.rows[k];
            for (var d = f = 0,
            c, b = m.cells.length; d < b; d++) c = m.cells[d],
            f += c.colSpan;
            f > l && (l = f)
        }
        return l
        ...
        return {
            ...
            onLoad: function() {
                ...
                function() {
                    ...
                    b && b.setValue(a, !0);
                    a = this.getStyle("height", ""); (b = e.getContentElement("info", "txtHeight")) && b.setValue(a, !0)
                })
            },
            onShow: function() {
            ...
            onOk: function() {
                ...
                if (b.info) {
                    ...
                    if (!c.$.tHead && ("row" == f || "both" == f)) {
                        ...
                        for (i = 0; i < h.getChildCount(); i++) g = h.getChild(i),
                    ...
                    if (!this.hasColumnHeaders && ("col" == f || "both" == f)) for (j = 0; j < c.$.rows.length; j++) g = new CKEDITOR.dom.element(c.$.rows[j].cells[0]),
                    g.renameNode("th"),
                    g.setAttribute("scope", "row");
                    if (this.hasColumnHeaders && !("col" == f || "both" == f)) for (i = 0; i < c.$.rows.length; i++) j = new CKEDITOR.dom.element(c.$.rows[i]),
                    "tbody" == j.getParent().getName() && (g = new CKEDITOR.dom.element(j.$.cells[0]), g.renameNode("td"), g.removeAttribute("scope"));
                ...
                if (this._.selectedElement) try {
                    e.selectBookmarks(d)
                } catch(n) {} else a.insertElement(c),
                setTimeout(function() {
                    ...
                    children: [{
                        ...
                        commit: function(e, d) {
                            if (this.isEnabled()) {
                                ...
                                if (c) 0 < b.count() ? (b = b.getItem(0), b.setHtml("")) : (b = new CKEDITOR.dom.element("caption", a.document), d.getChildCount() ? b.insertBefore(d.getFirst()) : b.appendTo(d)),
