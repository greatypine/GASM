...
(function($) {
	...
	function bindTreeNodes(setting, treeObj) {
		...
		treeObj.bind(ZTREE_CHANGE, function (event, treeId, treeNode,checked) {
		  if ((typeof setting.callback.change) == "function") setting.callback.change(event, treeId, treeNode,checked);
		});
	...
	//改变节点的可选状态
	/*function setNodeDisabled(treeNode,disabled){
	...
	*/
	...
	function showTree(setting, treeNode) {
		setting.disabled = treeNode.disabled;
		var node_Class = "tree-node";
		//获取父节点
		var p = treeNode.parentNode;
		if(p==null){
			node_Class = "tree-node-parent";
		}
		if (!p) {
			p = setting.treeObj;
		} else {
		...
		if(treeNode.status!=null&&treeNode.status!=""){
			statusHtml = '<span class="treeStatus'+treeNode.status+'">&nbsp;&nbsp;&nbsp;</span>';
		}
		if(treeNode.status=="0003"){
			treeNode.name = treeNode.name.replace("审批","审批查看");	
			treeNode.name = treeNode.name.replace("编制","查看");	
		}
		
		var html = "<li  id='" + treeNode.tId + "' class='"+node_Class+"'>" + "<button type=\"button\" id='" + treeNode.tId + IDMark_Switch + "' title='' onfocus='this.blur();'></button><button type=\"button\" id='" + treeNode.tId + IDMark_Icon + "' title='' onfocus='this.blur();'></button>&nbsp;";
	
		if(treeNode.status=="0002"||(treeNode.status=="0003"&&treeNode.id=='4')){
			html +=  "<span style='color:#000;' id='" + treeNode.tId + IDMark_Span + "'></span>&nbsp;&nbsp;";
			...
			if(treeNode.nodes==""){
				if(treeNode.url!=null){
					...
					col = "color:#00F;";
				}else if(treeNode.id=="36"&&setting.showGd!=false){
					col = "color:#00F;";
				}
			}else if(treeNode.id=="select"){
				col = "color:#00F;";
			}
			html +=  "<a  id='" + treeNode.tId + IDMark_A + "' onclick=\"" + (treeNode.click || '') + "\" ><span style='"+col+"' id='" + treeNode.tId + IDMark_Span + "'></span></a>";
		}
		html += statusHtml+"" + "<ul id='" + treeNode.tId + IDMark_Ul + "'></ul>" + "</li>";
		p.append(html);
		
		var switchObj = $("#" + treeNode.tId + IDMark_Switch);
		var IDMark_IconObj = $("#" + treeNode.tId + IDMark_Icon);
		...
		if (setting.checkable) {
			...
			var disabled = treeNode.disabled == null ? false : treeNode.disabled;
			
			var display = "display:inline";
			if(treeNode.showCheckBox==false){
				display = "display:none";
			}
			IDMark_IconObj.after("&nbsp;<BUTTON style='"+display+"'  type='BUTTON' ID='" + treeNode.tId + IDMark_Check + "' onfocus='this.blur();' ></BUTTON>");
			...
			if(isCheckedNode!=null){
				if(treeNode[setting.checkedCol]==false){
				/*	if(isCheckedNode(treeNode.related)){
						checkObj.attr("disabled",false);
					}*/
				}
				
				if(treeNode.must==true){
					treeNode.check_True_Full = false;
					checkObj.attr("disabled",true);
				}
			}
			...
			function() {
				...
				//触发 CheckBox 点击事件
				setting.treeObj.trigger(ZTREE_CHANGE, [setting.treeObjId, treeNode,treeNode[setting.checkedCol]]);
	...
	function setNodeLineIcos(setting, treeNode) {
		...
		if (treeNode.isParent) {
			...
			if(treeNode.status!=null&&treeNode.status!=""){
				icoObj.addClass("ico_" + FolderMark_Docu + "_" + treeNode.status);
			}else{
...
//		}
	...
	function setChkClass(setting, obj, treeNode) {
		...
		if(obj.attr("disabled")==true){
			obj.addClass("checkBoxDisabled_"+treeNode["checked"]);	
		}else{
			obj.removeClass("checkBoxDisabled");	
		}
	}
	...
	function getTreeNodesByParam(setting, treeNodes, key, value) {
		...
		var result = [];
		for (var i = 0; i < treeNodes.length; i++) {
			if(treeNodes[i][key]!=null){
				if (treeNodes[i][key].indexOf(value)!=-1) {
	...
	function zTreePlugin(){
		return {
			...
			setNodeDisabled: function(treeNode,disabled){
				var checkObj = $("#" + treeNode.tId + IDMark_Check);
				checkObj.attr("disabled",disabled);
				setting.checkStyle = Check_Style_Box;
				if(disabled==true){
					(treeNode["checked"] = false);
				}
				setChkClass(setting, checkObj, treeNode);
			},
			setNodeCheckDisabled: function(treeNode,disabled,checked){
				var checkObj = $("#" + treeNode.tId + IDMark_Check);
				checkObj.attr("disabled",disabled);
				setting.checkStyle = Check_Style_Box;
				if(disabled==true){
