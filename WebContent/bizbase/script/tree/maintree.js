...
(function($) {
	...
	function initTreeNodes(setting, level, treeNodes, parentNode) {
		...
		for (var i = 0; i < treeNodes.length; i++) {
			...
			node.check_False_Full = node.check_False_Full==null?true:node.check_False_Full;
	...
	function showTree(setting, treeNode) {
        ...
        if (setting.showIcon) {
            var html = "<li id='" + treeNode.tId + "' class='" + node_Class + "'>" + "<button type=\"button\" id='" + treeNode.tId + IDMark_Switch + "' title='' onfocus='this.blur();'></button>" + "<a id='" + treeNode.tId + IDMark_A + "' onclick=\"" + (treeNode.click || '') + "\" ><button type=\"button\" id='" + treeNode.tId + IDMark_Icon + "' title='' onfocus='this.blur();'></button><span id='" + treeNode.tId + IDMark_Span + "'></span></a>" + "<ul id='" + treeNode.tId + IDMark_Ul + "'></ul>" + "</li>";
            p.append(html);
        }
        else {
			var nWidth = 280 - ((treeNode.level + 1) * 20);
            if (clickflag) {
                //var html = "<li style='background:#D1D6F0;height:20px;vertical-align:middle;border-color: #999999 #999999 #999999 #999999;border-right: 1px solid #999999;border-style: solid; border-width: 1px;' id='" + treeNode.tId + "'>&nbsp;&nbsp;<span style='vertical-align:top;cursor:pointer;'><img onclick='toggle(\"maintree\",\"pic1\")' src='../scripts/images/22.png' align='absmiddle' id='pic1' /></span>&nbsp;&nbsp;<font style='font-size:13px;font-weight:bold;cursor:text'>"+treeNode.name+"</font>" + "</li><div id='maintree' style='overflow:auto;'><ul id='" + treeNode.tId + IDMark_Ul + "'></ul></div>";
				var html = "<li id='" + treeNode.tId + "'><a id='" + treeNode.tId + IDMark_A + "' onclick=\"" + (treeNode.click || '') + "\" ><i class='fa fa-circle-o'></i>"+treeNode.name+"</a>"
					+ "<ul id='" + treeNode.tId + IDMark_Ul + "' class='treeview-menu'></ul>" + "</li>";
				p.append(html);
                switchNode(setting, treeNode);
                clickflag = false;
            }else{

                var html = "<li id='" + treeNode.tId + "'><a href='#' id='" + treeNode.tId + IDMark_A + "' onclick=\"" + (treeNode.click || '') + "\" ><i class='fa fa-circle-o' style='cursor: hand'></i>"+treeNode.name+"</a>"
					+ "<ul id='" + treeNode.tId + IDMark_Ul + "' class='treeview-menu' ></ul>" + "</li>";
                p.append(html);	
			}
        }
	...
	function setNodeLineIcos(setting, treeNode) {
		...
		if (setting.showIcon) {
			...
			if (treeNode.icon) 
				icoObj.attr("style", "background:url(" + treeNode.icon + ") 0 0 no-repeat;");
			else 
				icoObj.attr("style", "");
		}else{
			icoObj.attr("class", (treeNode.iconSkin ? treeNode.iconSkin : ""));
			if (treeNode.isParent) {
				var tmpOpen = (treeNode.open ? ("_" + FolderMark_Open) : ("_" + FolderMark_Close));
				switchObj.attr("class", switchObj.attr("class") + tmpOpen);
				switchObj.parent().css('background','#E4E6F5');
				switchObj.css('width','22px');
				icoObj.remove();
			}
			else {
				switchObj.attr("class", switchObj.attr("class") + "_" + FolderMark_Docu);
				icoObj.addClass("ico_" + FolderMark_Docu);
			    switchObj.css('width','0px');
			}
			if (treeNode.icon) 
				icoObj.attr("style", "background:url(" + treeNode.icon + ") 0 0 no-repeat;");
			else 
	...
	function selectNode(setting, treeNode) {
		...
		if (setting.editable && treeNode.editNameStatus) {
			...
			if(setting.showIcon){
				...
				$("#" + treeNode.tId + IDMark_A).parent().addClass(Class_CurSelectedNode_Edit);
			}
			...
			if (setting.showIcon) {
				...
				$("#" + treeNode.tId + IDMark_A).parent().addClass(Class_CurSelectedNode);
			}
		}
		addTreeDom(setting, treeNode);
