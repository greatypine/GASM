...
function formatDate(date, format, dateF, ctx){
  var oDate = date instanceof Date ? date : null;
  var re;
  var retString = "";
  if (oDate ||
    (oDate = _validDate(date, "xs:dateTime")) ||
    ...
    for (var i=0; i < format.length;){
      ...
      switch(format.substr(i, 1)){
        ...
        case "d"://        day in month            (Number)            10
          var numDays = 0;
          while (format.substr(i, 1)=="d"){i++;numDays++}
          retString += (numDays > 1) ? pad(oDate.getDate()) : oDate.getDate();
          break;
        case "h"://        hour in am/pm (1~12)    (Number)            12
          var numHs = 0;
          while (format.substr(i, 1)=="h"){i++; numHs++}
...
function _monthNumber(name){//

        var month= 0;
        for (var i=0; i<gaLang[gsLang]["shortMonths"].length; i++)
        {
          var month_name=gaLang[gsLang]["shortMonths"][i];
          if (name.toLowerCase()==month_name.toLowerCase())
          {
            month=i;
...
function _validDate(date, format){
  var parts;
  var oDate = new Date(); // Mon Jan 1 00:00:00 UTC 1
      ...
      for (var i=0,j=0; i < format.length & oDate.toString() != "NaN";){
        ...
        switch(format.substr(i, 1)){
          ...
          case "M"://        month in year           (Text &amp; Number)     July &amp; 07
            ...
            if (i-t == 1)
            {
              month1=_getInt(date,s,1,2);
              if (month1 == null ) break;
              oDate.setMonth(month1-1); j = j+month1.length-1;
          ...
          case "d"://        day in month            (Number)            10
            ...
            if (i-t == 1)
            {
              day1=_getInt(date,s,1,2);
              if (day1 == null ) break;
              oDate.setDate(day1); j=j+day1.length-1;
            }
            else
            {
           ...
           if (i-t == 1)
            {
              ...
              var hr1=_getInt(date,s,1,2);
              if (hr1 == null ) break;
              oDate.setHours(hr1);j=j+hr1.length-1;  //alert(oDate.toLocaleString());
            }
            else
            {
           ...
           if (i-t == 1)
            {
              var hr1=_getInt(date,s,1,2);
              if (hr1 == null ) break;
              oDate.setHours(hr1);j=j+hr1.length-1;
            }
            else
            {
              oDate.setHours(date.substr(s,j-s));
          ...
          case "a"://        am/pm marker            (Text)              PM
            ...
            else if (date.substr(s,j-s) == "AM" || date.substr(s,j-s) == "am")
            {
              if (oDate.getHours() == 12)
              {
                oDate.setHours(0);
