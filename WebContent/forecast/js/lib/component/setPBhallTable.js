	...
	*/
	var createColumnCom = function(_this,_columnType,fieldPoint,tr,_data){
		...
		if(_columnType=="rownum"){
			...
			val = ((val==""||val==undefined)?"off":val);
			obj = $("<input bidTableFlag=true type='checkbox' name='"+_this.fieldName[fieldPoint]+"' value='"+val+"' />");
			var temp = _this.checkBoxIndex++;
			obj.bind("click",function(){
				obj.val(_this.checkBoxValues[temp]);
			})
				...
				for(var k=0;k<inputObj.length;k++){
					if((this.supportValueType.toUpperCase()).indexOf(inputObj[k].tagName)!=-1){
						if("INPUT" == inputObj[k].tagName ){
							...
							if(this.tableColumnsType[typeField]=="checkbox2" || $(inputObj[k]).attr("type") == "hidden" || $(inputObj[k]).attr("type") == "text"){
								trValue[this.fieldName[fieldPoint++]] = $(inputObj[k]).val();
								
							}else if(this.tableColumnsType[typeField]=="checkboxBoolean"){
								trValue[this.fieldName[fieldPoint++]] = $(inputObj[k]).attr("checked");
		...
		for(var i=0;i<_data.length;i++){
			...
			for(var j=0;j<tdArr.length;j++){
				...
				for(var k=0;k<inputObj.length;k++){
					if((this.supportValueType.toUpperCase()).indexOf(inputObj[k].tagName)!=-1){
						if("INPUT" == inputObj[k].tagName ){
							if(this.tableColumnsType[typePoint]!="checkbox2"&&this.tableColumnsType[typePoint]!="checkboxBoolean"){
								if($(inputObj[k]).attr("type") == "checkbox" ||$(inputObj[k]).attr("type") == "button" || $(inputObj[k]).attr("type") == "submit" || $(inputObj[k]).attr("type") == "reset"){
									continue;	
								}
							}else if(this.tableColumnsType[typePoint]=="checkboxBoolean"){
							...
							if($(inputObj[k]).attr("bidType")=="date"&&obj[this.fieldName[fieldPoint]]!=null){
								...
								if(this.tableColumnsType[typePoint]=="checkbox2") {
									if((obj[this.fieldName[fieldPoint]] != "off")) {
										if((obj[this.fieldName[fieldPoint]]==""||obj[this.fieldName[fieldPoint]]==undefined||obj[this.fieldName[fieldPoint]]=="null")) {
											$(inputObj[k]).val("off");
											fieldPoint++
										} else {
											$(inputObj[k]).attr("checked","checked");
						...
						}else if(this.tableColumnsType[typePoint] == "detail"  ){
							fieldPoint++;
						}
